...
matplotlib.pyplot.hist(Change, bins = 50)#histogram
pyplot.show()
#plot of change vs PE ratio
matplotlib.pyplot.plot(Change, PEratio[:T-horizon], 'go')
pyplot.show()
#plot of change vs Shiller PE ratio
matplotlib.pyplot.plot(Change, ShillerPEratio[:T-horizon], 'ro')
pyplot.show()

#regression every 5 years, data cannot overlap
indep_steps = int(T/horizon)
sampled = PEratio[0::horizon]
print(scipy.stats.linregress(sampled[:-1], Change[0::horizon]))
